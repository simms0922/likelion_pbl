  ...
  const handleFillRandom = async () => {
    try {
      ...
      setFormData({
        ...
        detail: '비동기(async/await)로 받아온 데이터를 UI에 반영하는 흐름을 이해하려고 합니다. 목표는 "데이터가 바뀌면 UI를 다시 그리는 구조"를 체득하는 것입니다.',
        email: transformed.email,
        phone: apiUser.phone,
        website: `https://example.com/${transformed.name.replace(/\s/g, '').toLowerCase()}`,
        ...
        <section className="inline-form-section" style={{ backgroundColor: 'white', padding: '30px', borderRadius: '12px', border: '1px solid #e5e7eb', marginBottom: '30px' }}>
          <form onSubmit={handleAddSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label style={{ fontSize: '13px', fontWeight: 'bold', color: '#555' }}>관심 기술 (쉼표로 구분)</label>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label style={{ fontSize: '13px', fontWeight: 'bold', color: '#555' }}>한 줄 소개 (요약 카드)</label>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label style={{ fontSize: '13px', fontWeight: 'bold', color: '#555' }}>자기소개 (상세 카드)</label>
              <textarea rows="4" value={formData.detail} onChange={(e) => setFormData({ ...formData, detail: e.target.value })} style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '6px', resize: 'vertical' }}></textarea>
            </div>
