...
function List({ lionList, setLionList, fetchRandomLions, fetchStatus, lastRequest }) {
  // ⭐ 핵심: URL 주소창을 읽고 쓰는 도구!
  const [searchParams, setSearchParams] = useSearchParams();
  const navigate = useNavigate(); // 페이지 이동 도구

  // 1. URL에서 옵션 값 읽어오기 (없으면 기본값 세팅)
  const filterPart = searchParams.get('part') || 'all';
  const sortOrder = searchParams.get('sort') || 'newest';
  const searchTerm = searchParams.get('q') || '';

  // 2. 옵션이 바뀔 때마다 URL 주소창을 업데이트하는 함수
  const updateParams = (key, value) => {
    const newParams = new URLSearchParams(searchParams);
    
    // 요구사항: 기본값이면 URL에서 꼬리표(파라미터) 지워버리기!
    if (value === 'all' || value === 'newest' || value === '') {
      newParams.delete(key);
    } else {
      newParams.set(key, value);
    }
    setSearchParams(newParams); // URL 갱신! -> 화면이 알아서 다시 그려짐
  };

  // 3. 화면에 보여줄 명단 계산 (URL 조건 기준)
  ...
  return (
    <main className="container">
      <section className="control-section">
        ...
        {/* ⭐ 기존의 onChange를 updateParams 함수로 바꿔서 URL에 적용되게 만듦 */}
      ...
      {/* 요약 카드 목록 (클릭하면 상세 페이지로 이동하도록 감싸줌!) */}
